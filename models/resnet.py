...
# here is a rectified resnet for cifar-10
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, low_dim=128, in_channel=3, width=1):
        ...
        """
        self.conv1 = nn.Conv2d(in_channel, 64, kernel_size=3, stride=1, padding=3,
                               bias=False)
        self.bn1 = nn.BatchNorm2d(64)
        self.relu = nn.ReLU(inplace=True)
    ...
    def forward(self, x, layer=7):
        ...
        if layer == 5:
            return x
        x = self.avgpool(x) if x.shape[-1] > 1 else x
        x = x.view(x.size(0), -1)
        if layer == 6:# for linear evaluation
            return x
        x = self.fc(x)
        #x = self.l2norm(x)
